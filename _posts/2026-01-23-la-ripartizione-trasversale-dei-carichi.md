...
meta: "Archivio StruHub Â· 816 parole circa"
---
...
La ripartizione trasversale dei carichi in impalcati a travi con soletta collaborante puÃ² essere ricondotta alla risposta di una piastra ortotropa equivalente: l'impalcato discreto (travi longitudinali, eventuali traversi, soletta) viene rappresentato come mezzo continuo con rigidezze per unitÃ  di lunghezza nelle due direzioni principali. La formulazione di Guyon-Massonnet-BareÃ…Â¡ (GMB) permette di combinare uno sviluppo modale lungo la luce con funzioni di ripartizione trasversale per stimare, in modo coerente e rapido, gli effetti (momento/taglio) su ciascuna trave.

Ipotesi alla base della formulazione

- Equivalenza continua travi-soletta: l'impalcato discreto Ã¨ rappresentato come piastra ortotropa con rigidezze distribuite per unitÃ  di lunghezza nelle due direzioni principali (longitudinale e trasversale).

- LinearitÃ  elastica: comportamento lineare dei materiali; sovrapposizione degli effetti (principio di sovrapposizione) nelle combinazioni di carico.

- Sezioni piane: in direzione longitudinale (compatibilitÃ  ala-anima) e aderenza perfetta soletta-travi (niente scorrimenti relativi).
...
- Valutazione trasversale tramite K: la distribuzione trasversale si basa sulle funzioni limite k 0 (piastra non torsorigida) e k 1 (piastra isotropa), con interpolazione non lineare k Î± in funzione di Î± e Ï‘.

- Larghezza attiva convenzionale: l'ampiezza efficace 2b include l'eventuale contributo (ridotto) degli sbalzi laterali.

- Torsione "globale": l'effetto torsionale Ã¨ sintetizzato in Î± mediante Ï G, G equivalenti (Saint-Venant), senza dettagli locali di distorsione o warping complesso.
...
- Rigidezze torsionali per unitÃ  di lunghezza nelle due direzioni (con G=E/[2(1+v)]):

La risposta trasversale Ã¨ governata da due parametri adimensionali:

dove 2b Ã¨ la larghezza attiva convenzionale e L la luce. Torna utile introdurre:

Interpretazione: Î± misura l'efficacia torsionale globale della piastra equivalente (da Î±=0, non torsiorigida, a Î±=1, isotropa, mentre Î¸ lega la snellezza trasversale alla geometria (b/L) e al rapporto di rigidezze Ï/.

Sviluppo modale dei carichi lungo la luce

Assumendo appoggi semplici in direzione longitudinale, il carico p(x) Ã¨ sviluppato in serie di Fourier sinusoidale:
...
Ripartizione trasversale: funzioni k 0, k 1 e interpolazione k Î±

L'effetto in una trave posta alla coordinata "y" di un'azione applicata alla coordinata trasversale "e" si ottiene moltiplicando l'ampiezza modale a m(e) per un coefficiente di ripartizione k(y,e) che dipende da Ï‘,Î± e da (y,e).

Caso non torsiorigido (Î±=0): k 0
...
Caso isotropo (Î±=1): k 1

Con

Interpolazione torsionale: k Î±

Si adotta una interpolazione non lineare fra i due limiti:

con esponente Î· Ï‘ calibrato (ad es. Î·=0.05 per Ï‘ \leq0.10; Î·=0.50 per 1.00 \leq Ï‘ \leq2.00; altrimenti:

Ricostruzione della risposta della piastra e proiezione su travi

Per il modo m si pone Î± m =m Ï€/L. La deformata modale della piastra Ã¨:

La pesante penalizzazione Î± m ^-4 garantisce la convergenza per m elevati.

Per i diagrammi 1D lungo la luce (schema SS), i contributi modali sono:

Con piÃ¹ corsie, le azioni si sommano e si definisce la media per trave:
...
- Impalcati curvi o con planimetria complessa: la formulazione GMB nasce per impalcati rettilinei; su tracciati in curva (o con variazioni brusche di direzione) la ripartizione puÃ² risultare non rappresentativa senza correzioni dedicate o un modello FEM di piastra/trave.

- Cassoni molto rigidi a torsione (alti Î±): per ponti a cassone chiuso/mono-cellula/multi-cellula, la rigidezza torsionale puÃ² alterare sensibilmente la ripartizione; la sola interpolazione k Î± puÃ² richiedere calibrazione o l'impiego di estensioni specifiche.
...
- Sbalzi laterali estesi: se la larghezza a sbalzo Ã¨ grande e la sua efficacia non Ã¨ ridotta in b, il metodo puÃ² sovrastimare la partecipazione degli sbalzi; opportuno introdurre coefficienti di efficacia dedicati.
